...
if(file_exists(JPATH_SITE.DS.'components'.DS.'com_iproperty'.DS.'iproperty.php')){
	...
	require_once(JPATH_SITE.DS.'modules'.DS.'mod_ice_carousel'.DS.'libs'.DS.'ip_helpers'.DS.'property.php');
}
$lang =& JFactory::getLanguage();
$lang->load( "com_iproperty" );
if( !class_exists('IceIpropertyHelperRoute')){
	class IceIpropertyHelperRoute extends ipropertyHelperRoute{
		//property
		public static function getPropertyRoute($id, $cat = null)
		{
			if($cat){
				$cats = (int)$cat;
			...
			//Create the link
			$id = (parent::_sef()) ? $id : (int)$id;
			$link = 'index.php?option=com_iproperty&view=property&id='.$id;
			
			if ($item = parent::_findItem($needles)) {
				$link .= '&Itemid='.$item;
			}
			elseif( $item = parent::_findItem($needles2)){
				$link .= '&Itemid='.$item;
			}
			elseif ($item = parent::_findItem()) {
				$link .= '&Itemid='.$item;
			}
			return $link;
		}
...
if( !class_exists('IceGroupCarouselIp') ) {
  /**
   * IceGroupAccordionJoomshopping Class  extends the LofSliderGroupBase Class
   */   
   class IceGroupCarouselIp extends IceGroupBase{
    ...
    public function getListByParameters( $params ){
      if( !IceGroupCarouselIp::isIpExisted() ){
        return array();
      }
	...
	{
		$db =& JFactory::getDBO();
        $property = new lofCarouselHelperProperty($db);
    ...
    public function __getList( $params ){
      ...
      $now    = $date->toMySQL();
		...
		elseif( $source == "ip_agents"){
			...
			if(!empty($agents)){
				if( $filter_type ){
					$where[] = 'am.prop_id IN ('.$agents.')';
				}
				else{
					$where[] = 'am.prop_id NOT IN ('.$agents.')';
				}
			}
		}
	 ...
	 die();
	 */
	  $rows = IceGroupCarouselIp::getPropertiesList($where,0,$limit, $ordering, $dir);
	 ...
	 if( $rows ){
            foreach ( $rows as $row )
            {
				...
				$lists[$i]->link            = JRoute::_( IceIpropertyHelperRoute::getPropertyRoute($row->id, $first_cat));
                ...
                $prepared_text = $row->short_description ? IceGroupCarouselIp::prepareContent($row->short_description, $descriptionMaxChars) : IceGroupCarouselIp::prepareContent($row->description, $descriptionMaxChars);
