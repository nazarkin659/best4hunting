...
class Sh404sefClassRouterInternal extends JRouterSite
{
	...
	public function parseRule(&$jRouter, &$uri)
	{
		 ...
		 * EDIT: no, we can't do the latter, that would prevent switching back language on home page
		 *
		 * NOTE: we must restore the leading lang code BEFORE _checkAliases and _checkHomePageRdirects
		 * as they expect to work with the actual requested url.
		 *
		 */
	...
	protected function _buildSefRouteInternal(&$uri)
	{
		...
		// restore language information that has been destroyed
		// by the language filter build rules
		$path = $uri->getPath();
		$path = trim(str_replace('index.php', '', $path), '/');

		// is there a language code in the remains of the path?
		$languagesBySefCode = JLanguageHelper::getLanguages('sef');
		if (isset($languagesBySefCode[$path]))
		{
			// languagefilter also removed Itemid on homepages
			// in such case, we're going to restore it from
			// menu links
			$vars = $uri->getQuery(true);
			if (empty($vars))
			{
				$homes = $pageInfo->homeLinks;
				$tag = $languagesBySefCode[$path]->lang_code;
				if (isset($homes[$tag]))
				{
					parse_str(str_replace('index.php?', '', $homes[$tag]), $vars);
					$uri->setQuery($vars);
				}
			}
			// there was language info, it was removed by langaugefilter plugin
			// let's put it back
			$uri->setPath('index.php');
			$uri->setVar('lang', $path);
		}
	...
	protected function _getSimpleSefLanguageSefCode($url)
	{
		...
		if (is_null($prefixes))
		{
			...
			foreach ($languages as $sefCode => $language)
			{
				if ($sefCode == $default)
				{
					$prefixes[$sefCode] = $app->getLanguageFilter() && Sh404sefHelperLanguage::getInsertLangCodeInDefaultLanguage() ? $sefCode . '/component/com_'
						: 'component/com_';
				}
				else
				{
