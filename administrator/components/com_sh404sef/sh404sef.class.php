...
function shInsertIsoCodeInUrl($compName, $shLang = null)
{
	...
	if (empty($compName) || !$sefConfig->shInsertLanguageCode // if no compname or global param is off
		...
		|| ($sefConfig->shLangInsertCodeList[$shLang] == 0 
				&& Sh404sefHelperLanguage::getDefaultLanguageTag() == $shLang
...
function shGetMenuItemLanguage($id)
{
	...
	if (!empty($menu))
	{
		...
		if (!empty($item))
		{
			$language = $item->language == '*' ? Sh404sefHelperLanguage::getDefaultLanguageTag() : $item->language;
		}
	}
	return $language;
}
...
function shSefRelToAbs($string, $shLanguageParam, &$uri, &$originalUri)
{
	...
	$id = Sh404sefHelperUrl::getUrlVar($string, 'Itemid', JRequest::getInt('Itemid'));
	...
	{
		$string .= '&amp;lang=' . Sh404sefHelperLanguage::getUrlCodeFromTag($shLanguage);
		return $string;
	}
			...
			switch ($shComponentType)
			{
				...
				// check for custom urls
					...
					if ($urlType != sh404SEF_URLTYPE_CUSTOM)
					{
						...
						$languageTag = empty($uriVars['lang']) ? Sh404sefFactory::getPageInfo()->currentLanguageTag : Sh404sefHelperLanguage::getLangTagFromUrlCode($uriVars['lang']); 
