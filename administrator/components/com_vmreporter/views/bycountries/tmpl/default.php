...
$getCountries = array();
$getOrders = array();
$getCountries[0] = JText::_('JALL');
$getOrders[0] = JText::_('JALL');
foreach($this->getCountries as $Country):
	$getCountries[$Country->virtuemart_country_id] = $Country->country_name;
endforeach;
...
$html = array();
$orders = array();
foreach($getCountries as $id => $Country) {
	$html[] = JHTML::_('select.option', $id, $Country);
}
...
<div class="generate-report">
	...
	<div class="reporter-inner">		
		<form action="<?php echo JRoute::_('index.php?option=com_vmreporter&view=bycountries');?>" method="post" name="adminForm1" id="adminForm1">
			<table class="report-table">
				<thead>
					<tr>
						<th width="45%">
							<?php echo JText::_('COM_VMREPORTER_COUNTRIES')?>
						</th>
						<th width="25%">
				...
				<tbody>					
					<tr>
						<td>						
							<?php echo  JHTML::_ ('select.genericlist', $html, 'countries[]','class="inputbox first chzn-select" multiple="true" size="1"', 'value', 'text', $this->SelectedCountries);?>						
						</td>
						<td>
			...
			<input  type="hidden" name="view" value="bycountries"/>
		</form>		
	</div>	
</div>
...
<div class="available-reports">
	...
	<div class="reporter-inner">  
		<form action="<?php echo JRoute::_('index.php?option=com_vmreporter&view=bycountries');?>" method="post" name="adminForm" id="adminForm">			
			<table class="adminlist">
				...
				<tbody>
					<?php foreach($this->reports as $key=>$value):?>
						...
						<td class="center">
							<a href="<?php echo JRoute::_('index.php?option=com_vmreporter&view=bycountry&id='.$value->id)?>"><?php echo $value->created_on?></a>
						</td>
						<td class="left">
							<div class="queries">
							<?php $decode_countries = json_decode($value->report_query);
							
								  //Countries//							
								  echo '<div class="query"><strong>'.JText::_("COM_VMREPORTER_COUNTRIES").':</strong> ';
								  $pCount = count($decode_countries->country);
								  $p = 1;
								  if($pCount):
								  	foreach($decode_countries->country as $country):
								  ...
								  $sCount = count($decode_countries->status);
								  $s = 1;
								  if($sCount):
								  	foreach($decode_countries->status as $status):
								  ...
								  echo $decode_countries->from.' '.JText::_("COM_VMREPORTER_TO").' '.$decode_countries->to;
								  echo "</div>";
							?>
							</div>
						</td>
						<td class="center" width="10%">
							<a href ="<?php echo JRoute::_('index.php?option=com_vmreporter&task=bycountries.createCSV&id='.$value->id)?>"><?php echo JText::_('COM_VMREPORTER_REPORT_DOWNLOAD') ?></a>
						</td>
						</tr>
					<?php endforeach;?>
				</tbody>
