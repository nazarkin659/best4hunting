 ...
 */
class VmreporterViewBycategories extends JView
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		$model = JModel::getInstance("bycategories","VmreporterModel");

		$mainframe = JFactory::getApplication();
		$SelectedCategories = $mainframe->getUserStateFromRequest('categories','categories',array('0'),'array' );
		$SelectedStauses = $mainframe->getUserStateFromRequest('status','status',array('0'),'array' );
		$this->assignRef('SelectedCategories',$SelectedCategories);		
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_VMREPORTER').': '.JText::_('COM_VMREPORTER_TITLE_BYCATEGORIES'), 'bycategories.png');

        $formPath = JPATH_COMPONENT_ADMINISTRATOR.'/views/byproduct';
		JToolBarHelper::deleteList('' , 'bycategories.delete');
	}
}
