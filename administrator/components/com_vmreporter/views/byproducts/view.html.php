 ...
 */
class VmreporterViewByproducts extends JView
{
	protected $items;
    ...
    VmreporterHelper::addSubmenu($view);
		$model = JModel::getInstance("byproducts","VmreporterModel");
		
		$mainframe = JFactory::getApplication();
		$SelectedProducts = $mainframe->getUserStateFromRequest('products','products',array('0'),'array' );
		$SelectedStauses = $mainframe->getUserStateFromRequest('status','status',array('0'),'array' );
		$this->assignRef('SelectedProducts',$SelectedProducts);		
		$this->assignRef('SelectedStauses',$SelectedStauses);
		
		$getProducts = $model->getProducts();
		$this->assignRef('getProducts', $getProducts);
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_VMREPORTER').': '.JText::_('COM_VMREPORTER_TITLE_BYPRODUCTS'), 'byproducts.png');

        //Check if the form exists before showing the add/edit buttons
        $formPath = JPATH_COMPONENT_ADMINISTRATOR.'/views/byproduct';
		JToolBarHelper::deleteList('' , 'byproducts.delete');
	}
}
