...
$getProducts = array();
$getOrders = array();
$getProducts[0] = JText::_('JALL');
$getOrders[0] = JText::_('JALL');
foreach($this->getProducts as $products):
	$getProducts[$products->virtuemart_product_id] = $products->product_name;
endforeach;
...
foreach($getProducts as $id => $product) {
	$html[] = JHTML::_('select.option', $id, $product);
}
...
<div class="generate-report">
	...
	<div class="reporter-inner">		
		<form action="<?php echo JRoute::_('index.php?option=com_vmreporter&view=byproducts');?>" method="post" name="adminForm1" id="adminForm1">
			<table class="report-table">
				<thead>
					<tr>
						<th width="45%">
							<?php echo JText::_('COM_VIRTUEMART_PRODUCT_NAME')?>
						</th>
						<th width="25%">
				...
				<tbody>					
					<tr>
						<td>						
							<?php echo  JHTML::_ ('select.genericlist', $html, 'products[]','class="inputbox first chzn-select" multiple="true" size="1"', 'value', 'text', $this->SelectedProducts);?>						
						</td>
						<td>
			...
			<input  type="hidden" name="view" value="byproducts"/>
		</form>		
	</div>	
</div>
...
<div class="available-reports">
	...
	<div class="reporter-inner">  
		<form action="<?php echo JRoute::_('index.php?option=com_vmreporter&view=byproducts');?>" method="post" name="adminForm" id="adminForm">			
			<table class="adminlist">
				...
				<tbody>
					<?php foreach($this->reports as $key=>$value):?>
						...
						<td class="center">
							<a href="<?php echo JRoute::_('index.php?option=com_vmreporter&view=byproduct&id='.$value->id)?>"><?php echo $value->created_on?></a>
						</td>
						<td class="left">
							<div class="queries">
							<?php $decoded_products = json_decode($value->report_query);
							
								  //PRODUCTS//							
								  echo '<div class="query"><strong>'.JText::_("COM_VIRTUEMART_PRODUCT_NAME").':</strong> ';
								  $pCount = count($decoded_products->products);
								  $p = 1;
								  if($pCount):
								  	foreach($decoded_products->products as $product):
								  ...
								  $sCount = count($decoded_products->status);
								  $s = 1;
								  if($sCount):
								  	foreach($decoded_products->status as $status):
								  ...
								  echo $decoded_products->from.' '.JText::_("COM_VMREPORTER_TO").' '.$decoded_products->to;
								  echo "</div>";
							?>
							</div>
						</td>
						<td class="center" width="10%">
							<a href ="<?php echo JRoute::_('index.php?option=com_vmreporter&task=byproducts.createCSV&id='.$value->id)?>"><?php echo JText::_('COM_VMREPORTER_REPORT_DOWNLOAD') ?></a>
						</td>
						</tr>
					<?php endforeach;?>
				</tbody>
