...
$getOrders = array();
$getManufacturers[0] = JText::_('JALL');
$getOrders[0] = JText::_('JALL');
foreach($this->getManufacturers as $Manufacturers):
	$getManufacturers[$Manufacturers->virtuemart_manufacturer_id] = $Manufacturers->mf_name;
endforeach;
...
$html = array();
$orders = array();
foreach($getManufacturers as $id => $Manufacturers) {
	$html[] = JHTML::_('select.option', $id, $Manufacturers);
}
...
<div class="generate-report">
	...
	<div class="reporter-inner">		
		<form action="<?php echo JRoute::_('index.php?option=com_vmreporter&view=bymanufacturers');?>" method="post" name="adminForm1" id="adminForm1">
			<table class="report-table">
				<thead>
					<tr>
						<th width="45%">
							<?php echo JText::_('COM_VMREPORTER_MANUFACTURER')?>
						</th>
						<th width="25%">
				...
				<tbody>					
					<tr>
						<td>						
							<?php echo  JHTML::_ ('select.genericlist', $html, 'manufacturers[]','class="inputbox first chzn-select" multiple="true" size="1"', 'value', 'text', $this->SelectedManufacturers);?>						
						</td>
						<td>
			...
			<input  type="hidden" name="view" value="bymanufacturers"/>
		</form>		
	</div>	
</div>
...
<div class="available-reports">
	...
	<div class="reporter-inner">  
		<form action="<?php echo JRoute::_('index.php?option=com_vmreporter&view=bymanufacturers');?>" method="post" name="adminForm" id="adminForm">		
			<table class="adminlist">
				...
				<tbody>
					<?php foreach($this->reports as $key=>$value):?>
						...
						<td class="center">
							<a href="<?php echo JRoute::_('index.php?option=com_vmreporter&view=bymanufacturer&id='.$value->id)?>"><?php echo $value->created_on?></a>
						</td>
						<td class="left">
							<div class="queries">
							<?php $decoded_manufacturers = json_decode($value->report_query);
							
								  //MANUFACTURERS//							
								  echo '<div class="query"><strong>'.JText::_("COM_VMREPORTER_MANUFACTURER").':</strong> ';
								  $mCount = count($decoded_manufacturers->manufcturers);
								  $m = 1;
								  if($mCount):
								  	foreach($decoded_manufacturers->manufcturers as $manufacturer):
								  ...
								  $sCount = count($decoded_manufacturers->status);
								  $s = 1;
								  if($sCount):
								  	foreach($decoded_manufacturers->status as $status):
								  ...
								  echo $decoded_manufacturers->from.' '.JText::_("COM_VMREPORTER_TO").' '.$decoded_manufacturers->to;
								  echo "</div>";
							?>
							</div>
						</td>
						<td class="center" width="10%">
							<a href ="<?php echo JRoute::_('index.php?option=com_vmreporter&task=bymanufacturers.createCSV&id='.$value->id)?>"><?php echo JText::_('COM_VMREPORTER_REPORT_DOWNLOAD') ?></a>
						</td>
						</tr>
					<?php endforeach;?>
				</tbody>
