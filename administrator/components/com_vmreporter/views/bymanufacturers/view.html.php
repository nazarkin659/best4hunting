 ...
 */
class VmreporterViewBymanufacturers extends JView
{
	protected $items;
	...
	public function display($tpl = null)
	{
		...
		$model = JModel::getInstance("bymanufacturers","VmreporterModel");

		$mainframe = JFactory::getApplication();
		$SelectedManufacturers = $mainframe->getUserStateFromRequest('manufacturers','manufacturers',array('0'),'array' );
		$SelectedStauses = $mainframe->getUserStateFromRequest('status','status',array('0'),'array' );
		$this->assignRef('SelectedManufacturers',$SelectedManufacturers);		
		$this->assignRef('SelectedStauses',$SelectedStauses);

		$getManufacturers = $model->getManufacturers();
		$this->assignRef('getManufacturers', $getManufacturers);
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_VMREPORTER').': '.JText::_('COM_VMREPORTER_TITLE_BYMANUFACTURERS'), 'by-manufacturers.png');

        //Check if the form exists before showing the add/edit buttons
        $formPath = JPATH_COMPONENT_ADMINISTRATOR.'/views/byproduct';
		JToolBarHelper::deleteList('' , 'bymanufacturers.delete');
	}
}
