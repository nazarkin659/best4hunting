 ...
 */
class VmreporterModelbymanufacturers extends JModelList {
	...
	function getManufacturers()
	{
		...
		VmConfig::loadConfig();
		$sql = "SELECT virtuemart_manufacturer_id,mf_name FROM #__virtuemart_manufacturers_".VMLANG;
		$db->setQuery($sql);
    ...
    $userInfoTable = ' LEFT JOIN `#__virtuemart_order_userinfos` as u on o.virtuemart_order_id = u.virtuemart_order_id';
		...
		$joinTables['manufactures'] = ' LEFT JOIN `#__virtuemart_product_manufacturers` as m ON i.virtuemart_product_id = m.virtuemart_product_id';
	...
	if(!in_array(0,$post['manufacturers'])){
		foreach($post['manufacturers'] as $manufacturer){
			$manufacturers[] = ' `m`.`virtuemart_manufacturer_id` = "'. $manufacturer .'"';
		}
	}
	if($manufacturers){
		$where[] = '(' . implode (' OR ',$manufacturers) . ')';
	}
	...
	function generateReports($post,$select,$joinedTables,$whereStatement)
	{
		...
		if (!class_exists( 'VmConfig' )) 
		   require(JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_virtuemart'.DS.'helpers'.DS.'config.php');		   
		VmConfig::loadConfig();
		$sql = "SELECT mf_name FROM #__virtuemart_manufacturers_".VMLANG."\n";
		$sql .= "WHERE virtuemart_manufacturer_id IN (".implode(",",$post['manufacturers']).")";
		$db->setQuery($sql);
		$manufacturer_name = $db->loadObjectList();
		...
		$PostDatas = array(
							"manufcturers" => $manufacturer_name,
							"status" =>	$status_name,
							"from" => $post['start'],
							"to" => $post['end']		
						  );
		...
		if(count($reports)):
		$sql = "INSERT into #__vmreporter_bymanufacturer SET ".$InsrtQuery;
		//print($sql);exit;
		$db->setQuery($sql);
		$db->query();
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT count(*) from #__vmreporter_bymanufacturer WHERE state = 1";
		$db->setQuery($sql);
		$count = $db->loadResult();
		return $count;
	}
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT * from #__vmreporter_bymanufacturer WHERE state = 1 ORDER BY id DESC";
	...
	function createCSV($id)
	{
		...
		$db = JFactory::getDBO();
		$sql = "SELECT report_details from #__vmreporter_bymanufacturer WHERE id =".$id;
		$db->setQuery($sql);
		$reports = $db->loadObjectList();
		$reports = $reports[0];
