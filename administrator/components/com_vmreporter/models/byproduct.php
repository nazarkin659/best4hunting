 ...
 * Vmreporter model.
 */
class VmreporterModelbyproduct extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Byproduct', $prefix = 'VmreporterTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_vmreporter.byproduct', 'byproduct', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_vmreporter.edit.byproduct.data', array());
	...
	protected function prepareTable(&$table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__vmreporter_byproduct');
				$max = $db->loadResult();
	...
	public function getDataofReport($id,$SelectedColoumns)
	{
		...
		$ordering = array();
		$sql = "SELECT * FROM  #__vmreporter_byproduct WHERE id = ".$id;
		$db->setQuery($sql);
