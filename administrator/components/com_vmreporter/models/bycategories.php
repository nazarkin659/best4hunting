 ...
 */
class VmreporterModelbycategories extends JModelList {
    ...
    protected function getListQuery() {
        ...
        $query->from('`#__vmreporter_byproduct` AS a');
	...
	function getCategories()
	{
		...
		VmConfig::loadConfig();
		$sql = "SELECT virtuemart_category_id,category_name FROM #__virtuemart_categories_".VMLANG;
		$db->setQuery($sql);
    ...
    $userInfoTable = ' LEFT JOIN `#__virtuemart_order_userinfos` as u on o.virtuemart_order_id = u.virtuemart_order_id';
		...
		$joinTables['categories'] = ' LEFT JOIN `#__virtuemart_product_categories` as c ON i.virtuemart_product_id = c.virtuemart_product_id';
	...
	if(!in_array(0,$post['categories'])){
		foreach($post['categories'] as $category){
			$categories[] = ' `c`.`virtuemart_category_id` = "'. $category .'"';
		}
	}
	if($categories){
	...
	function generateReports($post,$select,$joinedTables,$whereStatement)
	{
		...
		if (!class_exists( 'VmConfig' )) 
		   require(JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_virtuemart'.DS.'helpers'.DS.'config.php');		   
		VmConfig::loadConfig();
		$sql = "SELECT category_name FROM #__virtuemart_categories_".VMLANG."\n";
		$sql .= "WHERE virtuemart_category_id IN (".implode(",",$post['categories']).")";
		$db->setQuery($sql);
		...
		if(count($reports)):
		$sql = "INSERT into #__vmreporter_bycategory SET ".$InsrtQuery;
		//print($sql);exit;
		$db->setQuery($sql);
		$db->query();
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT count(*) from #__vmreporter_bycategory WHERE state = 1";
		$db->setQuery($sql);
		$count = $db->loadResult();
		return $count;
	}
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT * from #__vmreporter_bycategory WHERE state = 1 ORDER BY id DESC";
	...
	function createCSV($id)
	{
		...
		$db = JFactory::getDBO();
		$sql = "SELECT report_details from #__vmreporter_bycategory WHERE id =".$id;
		$db->setQuery($sql);
		$reports = $db->loadObjectList();
		$reports = $reports[0];
				...
				foreach($attrs[0] as $attr) {
					if(is_object($attr)) {
						foreach($attr as $atr) {
							if(is_object($atr)) {
								foreach($atr as $opt) {
									$dataAttribute = str_replace('</p>', '', str_replace('<p>','', str_replace('<span class="costumValue" >',': ', str_replace('<span class="costumTitle">','', str_replace('</span>','', $opt))))).'; ';
									$dataAttribute .= str_replace(',', '.', $dataAttribute);
								}
							}
						}
					} else {
						$dataAttribute = str_replace('</p>', '', str_replace('<p>','', str_replace('<span class="costumValue" >',': ', str_replace('<span class="costumTitle">','', str_replace('</span>','', $attr))))).'; ';
						$dataAttribute .= str_replace(',', '.', $dataAttribute);
					}	
				}
			}
