 ...
 */
class VmreporterModelbyproducts extends JModelList {
	...
	function getProducts()
	{
		...
		VmConfig::loadConfig();
		$sql = "SELECT product.virtuemart_product_id,prodengb.product_name,product.product_sku from #__virtuemart_products_".VMLANG." as prodengb INNER JOIN #__virtuemart_products as product ON prodengb.virtuemart_product_id = product.virtuemart_product_id";
		$db->setQuery($sql);
	...
	if(!in_array(0,$post['products'])){
		foreach($post['products'] as $product){
			$products[] = ' `i`.`virtuemart_product_id` = "'. $product .'"';
		}
	}
	if($products){
		$where[] = '(' . implode (' OR ', $products) . ')';
	}
    ...
    //print($sql);exit;
		...
		if (!class_exists( 'VmConfig' )) 
		   require(JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_virtuemart'.DS.'helpers'.DS.'config.php');		   
		VmConfig::loadConfig();
		$sql = "SELECT product_name FROM #__virtuemart_products_".VMLANG."\n";
		$sql .= "WHERE virtuemart_product_id IN (".implode(",",$post['products']).")";
		$db->setQuery($sql);
		$product_name = $db->loadObjectList();	
		...
		if(count($reports)):
		$sql = "INSERT into #__vmreporter_byproduct SET ".$InsrtQuery;
		$db->setQuery($sql);
		$db->query();
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT count(*) from #__vmreporter_byproduct WHERE state = 1";
		$db->setQuery($sql);
		$count = $db->loadResult();
		return $count;
	}
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT * from #__vmreporter_byproduct WHERE state = 1 ORDER BY id DESC";
	...
	function createCSV($id)
	{
		...
		$db = JFactory::getDBO();
		$sql = "SELECT report_details from #__vmreporter_byproduct WHERE id =".$id;
		$db->setQuery($sql);
		$reports = $db->loadObjectList();
		$reports = $reports[0];
