 ...
 * Vmreporter model.
 */
class VmreporterModelbycountry extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Bycountry', $prefix = 'VmreporterTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_vmreporter.bycountry', 'bycountry', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_vmreporter.edit.bycountry.data', array());
	...
	protected function prepareTable(&$table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__vmreporter_bycountry');
				$max = $db->loadResult();
	...
	function getDataofReport($id,$SelectedColoumns)
	{
		$db = JFactory::getDBO();
		$FilterBy = JRequest::getVar('filterby', 'order_number');
		$FilterOrder = JRequest::getVar('dir', 'asc');
		$ordering = array();
		$sql = "SELECT * FROM  #__vmreporter_bycountry WHERE id = ".$id;
		$db->setQuery($sql);
