 ...
 */
class VmreporterModelbycustomers extends JModelList {
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT virtuemart_user_id,name FROM #__virtuemart_userinfos";
		$db->setQuery($sql);
    ...
    $selectFields[] = " replace(concat_ws(' ', u.title, u.first_name, u.middle_name, u.last_name), '  ', ' ') AS customer_name";
		...
		$joinTables['users'] = ' LEFT JOIN `#__virtuemart_order_userinfos` as u ON o.virtuemart_order_id = u.virtuemart_order_id';
	...
	if(!in_array(0,$post['users'])){
		foreach($post['users'] as $user){
			$users[] = ' `u`.`virtuemart_user_id` = "'. $user .'"';
		}
	}
	if($users){
		$where[] = '(' . implode (' OR ',$users) . ')';
	}
	...
	function generateReports($post,$select,$joinedTables,$whereStatement)
	{
		...
		$userid = $user->id;
		$sql = "SELECT name FROM #__virtuemart_userinfos\n";
		$sql .= "WHERE virtuemart_user_id IN (".implode(",",$post['users']).")";
		$db->setQuery($sql);
		...
		if(count($reports)):
		$sql = "INSERT into #__vmreporter_bycustomer SET ".$InsrtQuery;
		//print($sql);exit;
		$db->setQuery($sql);
		$db->query();
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT count(*) from #__vmreporter_bycustomer WHERE state = 1";
		$db->setQuery($sql);
		$count = $db->loadResult();
		return $count;
	}
	...
	{
		$db = JFactory::getDBO();
		$sql = "SELECT * from #__vmreporter_bycustomer WHERE state = 1 ORDER BY id DESC";
	...
	function createCSV($id)
	{
		...
		$db = JFactory::getDBO();
		$sql = "SELECT report_details from #__vmreporter_bycustomer WHERE id =".$id;
		$db->setQuery($sql);
		$reports = $db->loadObjectList();
		$reports = $reports[0];
