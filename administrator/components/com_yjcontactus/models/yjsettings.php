<?php
/**
 * Settings model for YjContactUS Component
 * 
 * @package     
 ...
 * @subpackage Components
 */
class YjContactUSModelYjSettings extends JModel
{
	/**
	...
	function getfolder(){ 
		...
		$return = array("folder"=>$folder[0]);
		
		return $return;
	}

	/**
	 * Retrieves the list of folders from images folders
	 * @return array Array of objects containing the data from the database
	 */
	function getlist_folders(){ 
	...
	function getList()
	{
		...
		if (strlen($current) > 0) {
			...
			//$basePath = JPATH_ROOT.DS.$current;
		} else {
	...
	function save() {
		...
		if (!$row->store()) {
			...
			$msg = JText::_("COM_YJCONTACTUS_SUCC_UPLOAD_NEW_FOLDER");
		}
