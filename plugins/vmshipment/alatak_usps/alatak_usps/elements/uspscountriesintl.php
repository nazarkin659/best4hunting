 ...
 */
class JElementUspsCountriesintl extends JElement {
	...
	function fetchElement ($name, $value, &$node, $control_name) {
		...
		$query = 'SELECT `country_2_code` AS value, `country_name` AS text FROM `#__virtuemart_countries`
               		WHERE `published` = 1 AND `country_2_code` NOT IN (' . $domestic_countries_list . ') ORDER BY `country_name` ASC ';
