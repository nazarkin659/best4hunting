    ...
    protected function getInput() {
	   if(!defined('VMLANG')){
        ...
        $attr .= $this->element['onchange'] ? ' onchange="' . (string) $this->element['onchange'] . '"' : '';
                ...
                $db = & JFactory::getDBO();
                $db->setQuery("select * from #__virtuemart_categories_$lng lng left join #__virtuemart_categories cat on cat.virtuemart_category_id=lng.virtuemart_category_id where published=1");

                $options = array ();
                $rows = $db->loadAssocList();
                foreach ($rows as $row){
                        $options[]=array("name"=>$row["category_name"],"value"=>$row["virtuemart_category_id"]);
                }
